...
def termination_embed_sp(ctx: InteractionContext, sesh: QuizSessionSP, message: str) -> Embed:
    '''
    ...
    end_embed.set_author(icon_url=ctx.author.avatar.url,
                         name=f"Quiz for {ctx.author.display_name}")
    return end_embed
...
async def listen_quiz(ctx: InteractionContext, quiz_name: str, questions: list):
    ...
    for question_num in range(len(questions)):
        ...
        # Generate components
        if mcq:
            components = [Button(label=REACTIONS[i].upper(
            ), style=ButtonStyles.BLUE, custom_id=str(i)) for i in range(len(options))]
        else:
            components = [
                Button(label="✅", style=ButtonStyles.GREEN, custom_id="0")]
        ...
        if result_embed:
            ...
            msg = await component_ctx.send(embed=embeds,
        ...
        try:
            ...
            sesh.question.answered_index = int(
    ...
    for question_num in range(len(questions)):
        ...
        async def listen_button():
            while 1:
                ...
                if component_ctx.custom_id == "stop" and component_ctx.author == ctx.author:
                    await component_ctx.send("Stopping Quiz...", ephemeral=True)
                    return "stop"
                elif component_ctx.custom_id == "skip" and component_ctx.author == ctx.author:
                    ...
                    result_embed = embed_result_mp(
                        sesh, component_ctx.author.id)
