...
from dis_snek.models.discord_objects.components import Button, ButtonStyles, spread_to_rows
...
def termination_embed_sp(ctx, sesh: QuizSessionSP, message: str) -> Embed:
    '''
    Embed to terminate a quiz session
    '''
    finalscore = f"Final Score: {sesh.current_score}/{sesh.current_mcq} ="\
        f" {round(sesh.current_score/sesh.current_mcq*100)}%" if sesh.current_mcq else None

    end_embed = Embed(title=message,
                    description = ("\n"+finalscore) if finalscore else "", color=MaterialColors.RED)
    end_embed.set_author(icon_url=ctx.author.avatar.url, name=f"Quiz for {ctx.author.display_name}")
    return end_embed
...
def termination_embed_mp(sesh: QuizSessionMP, message: str) -> Embed:
    ...
    standings.description = "\n".join(
        [f"<@{id}>: {sesh.current_score[id]}/{sesh.current_mcq[id]} = "
            f"{round(sesh.current_score[id]/sesh.current_mcq[id]*100)}%" for id in sesh.current_mcq])
    return standings
...
def embed_result_sp(session: QuizSessionSP) -> Embed:
    ...
    if question.mcq:
        ...
        if question.answered_index == question.correct_index:
            ...
            e = Embed(
                color=MaterialColors.GREEN, title="Correct",
                description=textToEmoji(
                    REACTIONS[question.correct_index]) +
                " " + question.options[question.correct_index])
        else:
            e = Embed(
                color=MaterialColors.RED, title="Incorrect",
                description=textToEmoji(
                    REACTIONS[question.correct_index]) +
                " " + question.options[question.correct_index])
    ...
    for question_num in range(len(questions)):
        ...
        if mcq := bool(current_question[-1]):
            ...
            if correct_index > len(current_question) - 1:
                await ctx.send(f"Error, Question `{prompt}` has an invalid 'Correct Answer' label."
                               "Please make sure the correct answer is labelled as {A, B, C, D, etc.}")
                continue

            question_embed.description = desc_text \
                + "\n".join(f"{textToEmoji(REACTIONS[i])} {options[i]}"
            ...
            question_embed.description = desc_text
            question_embed.set_author(name="Press ✅ to see the answer",
                                      icon_url=ctx.author.avatar.url)

        # Generate components
        if mcq:
            components = [Button(label=REACTIONS[i].upper(), style=ButtonStyles.BLUE, custom_id=str(i)) for i in range(len(options))]
        else:
            components = [Button(label="✅", style=ButtonStyles.GREEN, custom_id="0")]

        components = spread_to_rows(
            *(components +
              [Button(label="🛑",
                      style=ButtonStyles.RED, custom_id="stop")])) 
        
        sesh.question = QuizQuestion(
                True, correct_index, 0, options
            )
        
        # Handle embeds
        if result_embed:
            next_embed = Embed(
                title=f"Next Question ({question_num+1}/{len(questions)})", color=MaterialColors.BLUE)
            embeds = [result_embed, next_embed, question_embed]
            msg = await used_component.context.send(embed=embeds,
                                                    components=components,
                                                    ephemeral=True)
        else:
            intro_embed = Embed(
                title=quiz_name, description=f"**Starting Quiz** -- {len(questions)} questions",
        ...
        try:
            ...
            if used_component.context.custom_id == "stop":
                stop_embed = termination_embed_sp(ctx, sesh, "Quiz Terminated. Enter a new link to start again!")
                await used_component.context.send(embed=stop_embed, ephemeral=True)
                return
            
            sesh.question.answered_index = int(used_component.context.custom_id)
            result_embed = embed_result_sp(sesh)

        except asyncio.TimeoutError:
            await ctx.send(content="**Quiz timed out after 120 seconds of inactivity.**", ephemeral=True)
            return

    stop_embed = termination_embed_sp(ctx, sesh,
                                   "Question deck has been exhausted. Enter a new link to start again!")
    await used_component.context.send(embeds=[result_embed, stop_embed], ephemeral=True)
    return
...
async def listen_quiz_mp(ctx: InteractionContext, quiz_name: str, questions: list):
    ...
    for question_num in range(len(questions)):
        ...
        question_embed = Embed(color=MaterialColors.BLUE)
        question_embed.set_footer("Press 🛑 to stop the quiz, or ⏭️ to skip to the next question.")
        if image_url := current_question[1]:
            question_embed.set_image(url=image_url)
        ...
        components = spread_to_rows(
            *(components +
              [Button(label="🛑",
                      style=ButtonStyles.RED, custom_id="stop"),
               Button(label="⏭️",
                      style=ButtonStyles.GRAY, custom_id="skip")]))
        ...
        if question_num > 0:
            ...
            msg = await ctx.channel.send(embed=embeds,
                                                    components=components)
        else:
            intro_embed = Embed(
                title=quiz_name, description=f"**Starting Multiplayer Quiz** -- {len(questions)} questions",
                color=MaterialColors.BLUE)
        ...
        async def listen_button():
            while 1:
                ...
                if used_component.context.custom_id == "stop" and used_component.context.author == ctx.author:
                    await used_component.context.send("Stopping Quiz...", ephemeral=True)
                    return "stop"
                elif used_component.context.custom_id == "skip" and used_component.context.author == ctx.author:
                    await used_component.context.send("Skipping Question...", ephemeral=True)
                    return "skip"
                elif used_component.context.author.id not in answered:
                    sesh.question.answered_index = int(used_component.context.custom_id)
                    # send query to author privately
                    result_embed = embed_result_mp(sesh, used_component.context.author.id)
                    answered.append(used_component.context.author.id)
                    await used_component.context.send(embed=result_embed, ephemeral=True)
                
        t = asyncio.create_task(listen_button())
        try:
            result = await asyncio.wait_for(t, timeout=30)
            if result == "stop":
                stop_embed = termination_embed_mp(sesh, "Quiz Terminated. Enter a new link to start again!")
